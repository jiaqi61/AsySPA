...
Asynchronous subgradient-push class for distributed optimization using column stochastic matrix.
...
class AsyGradientPush(PushSumOptimizer):
    ...
    def __init__(self, objective,
        ...
        # bounded activation time interval
        if delay is None:
    ...
    def minimize(self, print_info=False):
        ...
        while condition:
            ...
            if self.terminate_by_time is False:
                ...
                # Add artificial delays to model uneven updates and communication delays
                if current_time - loop_start_time < self.delay:
                    time.sleep(self.delay - (current_time - loop_start_time)) 
                # time.sleep(np.random.exponential(self.delay))
...
if __name__ == "__main__":
    ...
    def demo_ls(num_instances_per_node, num_features):
        ...
        To run the demo (openmpi installed), run the following:
            mpiexec -n $(num_nodes) -bind-to core -allow-run-as-root python asy_gradient_push.py
        """
        ...
        filepath = 'result/least_square/'+str(UID)+'.mat'
        sio.savemat(filepath, mdict={'A': a_m, 'b': b_v, 'itr': itr, 'time': t,
                                     'estimate': est}) 
    # Run a demo
    demo_ls(num_instances_per_node = 200, num_features = 10)
