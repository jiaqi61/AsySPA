...
TIMEOUT_PERIOD = 10 # maximum time in seconds to wait for messages (only valid in synch nets.)
...
#                                          [consensus_column[i]*ps_w], [ps_l]])
    ...
    def recieve_asynchronously(self):
        ...
        :rtype: dict('num_messages': int, 'ps_w': float, 'ps_n': np.array[float] or float, 'ps_l': int)
        """
        itr = 0
        ps_w = 0
        ps_n = 0
        ...
        for _ in range(self.in_degree):
            ...
            while COMM.Iprobe(source=MPI.ANY_SOURCE, status=info) is False:
                if time.time() > threshold:
                    print('encounter deadlocks!', flush=True)
                    break_flag = True
                    break
            if break_flag:
                break
    ...
    def gossip(self, gossip_value, ps_weight=1.0, ps_loop=None, just_probe=False, start_together=False):
        ...
        if (self.synch is True) and (self.terminate_by_time is True):
            warnings.warn("Use of synchronous gossip w/ time term. cond. may result in deadlocks.")
        ...
        while condition:
            ...
            #     COMM.Barrier()    # non-blocking barrier

            if self.synch is True:
                mpi_request = COMM.Ibarrier() # non-blocking barrier
                t_synch = time.time()
                while True:
                    if mpi_request.Test():  # barrier succeed
                        break
                    # if synch is true and terminate_by_time is true, some thread may end before others
                    # so the barrier will failed. To avoid this, we use timeout.
                    if time.time() - t_synch > TIMEOUT_PERIOD:
...
if __name__ == "__main__":
    ...
    def demo(gossip_value):
        ...
        # Initialize averager
        psga = PushSumGossipAverager(synch=False,
